...
The run stopped partway through epoch 5 when a Windows DataLoader worker exited unexpectedly. The completed epoch 4 checkpoint retained optimizer, EMA, and AMP scaler state. At the user's request, the run was resumed from `last.pt` on 2026-09-24 with its original batch 16 and two workers, without reducing load. The visible resume launcher is `scripts/resume_p2_visible.ps1`. If the worker failure recurs, reconsider memory and worker settings before another retry.

The same DataLoader worker failure recurred partway through epoch 11. Epoch 10 had completed and its `last.pt` retained optimizer, EMA, and scaler state. The run was resumed from this checkpoint with batch 8 and `workers=0` via `scripts/resume_p2_safe_visible.ps1`; all other checkpoint settings remain unchanged. The run directory name still says `b16`, but the physical batch changes at epoch 11. Any final P2-vs-baseline claim should use a fixed-setting rerun or clearly disclose this mixed-batch recovery.
